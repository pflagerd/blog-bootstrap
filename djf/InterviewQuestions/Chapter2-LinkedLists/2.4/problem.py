...
class SinglyLinkedListNode:
    ...
    @staticmethod
    def generate_arbitrary_list( numbers ):
        if not numbers:
            return None

        # Create the first (head) node
        n = numbers[0]
        head = SinglyLinkedListNode( n )
        current_node = head

        for n in numbers[1:] :
            current_node.next = SinglyLinkedListNode( n )
    ...
    def generate_djf(n: int):
        ...
        # print( f'Generated: "{head.to_string()}"' )
        return head
    ...
    def delete( delete_me_ptr ):
        if not delete_me_ptr or not delete_me_ptr.next:
            # 1. If we don't point to a node, there is nothing to do.
            # 2. If this node is the last node, there is nothing to do.
            return

        print( f'   ... delete_me_ptr points to value {delete_me_ptr.value}.' )
        current = delete_me_ptr
        # Visit each node from current to the end
        while current.next:
            print( f'   ... looping: current.value is {current.value}' )
            current.value = current.next.value
            print( f'   ... ... current.value changed to:  {current.value}' )
            # If we just copied the value of the last node, now drop the last node.
            #     ... then current.next.value was the last
            #     ... then current.next.next is None.
    ...
    def partition( pivot ):
#         head = self
#         # Nothing to do with a singleton list
#         if not head or not head.next:
#             return
# 
#         dummy_value = 123456789
#         # create a bookend for each list, with a dummy node 
#         smalls = SinglyLinkedListNode.generate( dummy_value ) 
#         bigs = SingleLinkedListNOde.generate( dummy_value )
# 
#         current = head
#         while current:
...
class ReturnKthToLastTests(unittest.TestCase):
    ...
    def test_generate_djf2(self):
        print(f"SinglyLinkedListNode.generate_djf(1).() == '{SinglyLinkedListNode.generate_djf(1).dumps()}'")
        self.assertEqual('[0]', SinglyLinkedListNode.generate_djf(1).to_string())
        print(f"SinglyLinkedListNode.generate_djf(2).dumps() == '{SinglyLinkedListNode.generate_djf(2).dumps()}'")
        # self.assertEqual('{"next": {"next": null, "value": 1}, "value": 0}', SinglyLinkedListNode.generate_djf(2).dumps())
        self.assertEqual( '[0, 1]', SinglyLinkedListNode.generate_djf(2).to_string())
        print(f"SinglyLinkedListNode.generate_djf(3).dumps() == '{SinglyLinkedListNode.generate_djf(3).dumps()}'")
        # self.assertEqual('{"next": {"next": {"next": null, "value": 2}, "value": 1}, "value": 0}', SinglyLinkedListNode.generate_djf(3).dumps())
        self.assertEqual( '[0, 1, 2]', SinglyLinkedListNode.generate_djf(3).to_string())
        print(f"SinglyLinkedListNode.generate_djf(4).dumps() == '{SinglyLinkedListNode.generate_djf(4).dumps()}'")
        # self.assertEqual('{"next": {"next": {"next": {"next": null, "value": 3}, "value": 2}, "value": 1}, "value": 0}', SinglyLinkedListNode.generate_F(4).dumps())
        self.assertEqual('[0, 1, 2, 3]', SinglyLinkedListNode.generate_F(4).to_string())

    def test_z2_djf_partition0(self):
        new_list = SinglyLinkedListNode.generate_arbitrary_list( [3, 5, 8, 5, 10, 2, 1] )
        print( f"[3, 5, 8, 5, 10, 2, 1] == 'SinglyLinkedListNode.generate_arbitrary_list( [3, 5, 8, 5, 10, 2, 1]'" )
        self.assertEqual( '[3, 5, 8, 5, 10, 2, 1]', new_list.to_string() )

    def test_z2_djf_partition1(self):
        new_list = SinglyLinkedListNode.generate_arbitrary_list( [3, 5, 8, 5, 10, 2, 1] )
        print( f"[3, 1, 2, 10, 5, 5, 8] == 'SinglyLinkedListNode.generate_arbitrary_list( [3, 5, 8, 5, 10, 2, 1] ).partition( 5 )'" )
        self.assertEqual( '[3, 5, 8, 5, 10, 2, 1]', new_list.to_string() )


    def test_z1_djf_delete_from_middle(self):
        print()
        print( "DJF 'delete_node' test:" )
        head = SinglyLinkedListNode.generate_djf(4)
        print( f"   generated: {head.dumps()}" )
        delete_me = head.next.next
        print( f"   'delete_me' value is {delete_me.value}" )
        print( f"   before: {head.dumps()}" )
        print( '   deleting the node... ' )
        SinglyLinkedListNode.delete( delete_me )
        print( f"   after: {head.dumps()}" )
        self.assertEqual('{"next": {"next": {"next": null, "value": 3}, "value": 1}, "value": 0}', head.dumps() )

    def test_z2_djf_delete_from_middle(self):
        print()
        print( "DJF 'delete_node' test 2:" )
        head = SinglyLinkedListNode.generate_djf(7)
        print( f"   generated: {head.dumps()}" )
        delete_me = head.next.next.next
        print( f"   'delete_me' value is {delete_me.value}" )
        print( f"   before: {head.dumps()}" )
        print( '   deleting the node... ' )
        SinglyLinkedListNode.delete( delete_me )
        print( f"   after: {head.dumps()}" )
        self.assertEqual('{"next": {"next": {"next": {"next": {"next": {"next": null, "value": 6}, "value": 5}, "value": 4}, "value": 2}, "value": 1}, "value": 0}',
                         head.dumps() )
